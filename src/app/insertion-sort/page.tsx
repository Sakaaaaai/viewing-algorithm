...
const InsertionSort = () => {
    ...
    for (let i = 1; i < n; i++) {
      ...
      while (j >= 0 && arrCopy[j] > key) {
        steps.push({
          array: [...arrCopy],
          highlightedLines: [7],
          description: `${arrCopy[j]} と ${key} を比較`,
          variables: { i, key, j, n },
          comparing: [j, j + 1],
          phase: "comparison"
        });
      ...
      steps.push({
        array: [...arrCopy],
        highlightedLines: [11],
        description: `key (${key}) を位置 ${j + 1} に挿入`,
        variables: { i, key, j: j + 1, n },
        inserted: j + 1,
        phase: "insert"
      });
  ...
  useEffect(() => {
    setSortingSteps(generateInsertionSortSteps(array));
  }, [array]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-blue-200 dark:from-blue-900 dark:to-blue-800 py-6">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h1 className="text-4xl font-bold text-blue-600 dark:text-blue-400 text-center">挿入ソート</h1>
        </motion.div>
        ...
        >
          <Card className="bg-white dark:bg-gray-800 shadow-lg">
            <CardHeader>
              <CardTitle className="text-2xl text-blue-600 dark:text-blue-400">挿入ソートとは</CardTitle>
              <CardDescription>シンプルで直感的なソートアルゴリズム</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex items-start">
                <div className="space-y-4">
                  <p>
                    挿入ソートは、配列を順番に走査し、各要素を適切な位置に挿入することでソートを行うアルゴリズムです。
                  </p>
                  <p>
                    カードゲームでの手札の並べ替え方に似ており、人間の直感的な動作を模倣しています。
                  </p>
                  <div>
                    <h3 className="font-semibold mb-2">アルゴリズムの特徴：</h3>
                    <ul className="list-disc list-inside space-y-1">
                      <li>実装が簡単で理解しやすい</li>
                      <li>小規模なデータセットやほぼソートされているデータに対して効率的</li>
                      <li>大規模なデータセットに対しては非効率（平均・最悪計算量がO(n²)）</li>
                      <li>安定ソート（同じ値の要素の相対的な順序が保たれる）</li>
                      <li>オンラインアルゴリズム（データを逐次的に処理できる）</li>
                    </ul>
                  </div>
                  <p>
                    バブルソートと同様に教育目的でよく使用されますが、大規模データには他の効率的なアルゴリズムが好まれます。
                  </p>
                </div>
              </div>
            </CardContent>
